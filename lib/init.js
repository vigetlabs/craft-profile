...
function init(cfg) {
  ...
  let jobsPerWorker = Math.ceil(totalJobs / config.workers)
  let normalize = path => config.baseUrl + path.replace(/^\/+/, '/')
  let paths = config.paths.map(path => new Array(config.iterations).fill(normalize(path)))

  // randomizing paths does something... maybe
  jobs = flatRandom(paths).map(job => ({
    url     : url,
    error   : null,
    time    : null,
    queries : null
  ...
  for (let i = 0; i < config.workers; i++) {
    let worker = spawn(path.resolve('bin', 'worker'), [], {
    ...
    worker.on('error', err => {
      console.log(`Failed to start child worker.`, err)
    })
...
function onChildData(job) {
  let table = new Table()
  table.push([job.time, job.queries, job.url])
...
function format(results) {
  let heading = `${config.iterations} Iteration(s) across ${config.workers} Worker(s): ${totalJobs} Total Jobs`
